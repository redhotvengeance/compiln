      ...
      for (var e = 0; e < pluginExtensions.length; e++) {
        ...
        (function(extension, sourcePath) {
          fs.exists(sourcePath, function(exists) {
            if (exists === true) {
              pluginOptions.versionedFile = module.exports.versionedFile;

              plugin.compile(sourcePath, pluginOptions, function(error, compiledData) {
                if (error) throw error;
                var versionedFileRegEx = /versionedFile`([\w\.'"\/-]+)`/i

                while (versionedFileRegEx.test(compiledData)) {
                  var regexData = versionedFileRegEx.exec(compiledData);
                  var pos = regexData.index;
                  var full = regexData[0];
                  var inner = regexData[1].replace(/'/g, '').replace(/"/g, '');
                  compiledData = compiledData.substring(0, pos) + module.exports.versionedFile(inner) + compiledData.substring(pos + full.length, compiledData.length);
                }
...
module.exports.versionedFile = function(filename) {
  if (filename.string) {
...
module.exports.compile = function(options) {
  ...
  return function(req, res, next) {
    req.app.locals({
      versionedFile: module.exports.versionedFile
    });
